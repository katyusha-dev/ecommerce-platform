...
namespace Katyusha\Services\FaceId\FaceId\Http\Controllers;
...
use M1guelpf\FaceId\FaceIdServiceProvider;
...
class FaceIdController
{
    public function createDetails(Request $request)
    {
        ...
        )->setAuthenticatorSelection(new Authenticator('platform'))->excludeCredentials($request->user()->webauthnCredentials->map(fn ($credential) => new Credential(Credential::CREDENTIAL_TYPE_PUBLIC_KEY, $credential['credId'], ['internal']))->toArray()), fn ($creationOptions) => Cache::put($this->getCacheKey(), $creationOptions->jsonSerialize(), now()->addMinutes(5)))->jsonSerialize();
    }
    ...
    public function create(Request $request, CredentialLoader $credentialLoader, RegistrationValidator $registrationValidator, CredentialRequest $credentialRequest)
    {
        ...
        if (! $creationOptions || ! $credentials instanceof RegistrationResponse) {
            throw new UnauthorizedException('FaceId: Failed validating request', 422);
        }

        try {
            $response = $registrationValidator->check($credentials, $creationOptions, $credentialRequest, [$creationOptions->getRp()->getId()]);
        } catch (InvalidArgumentException $e) {
            throw new UnauthorizedException('FaceId: Failed validating request', 422, $e);
        }
        ...
        cookie()->queue(FaceIdServiceProvider::FASTLOGIN_COOKIE, $credId, 1 * Carbon::DAYS_PER_YEAR * Carbon::HOURS_PER_DAY * Carbon::MINUTES_PER_HOUR);
    ...
    public function loginDetails(Request $request)
    {
        return tap(
            LoginRequest::create(random_bytes(16))
                ...
                ->allowCredential(new Credential(Credential::CREDENTIAL_TYPE_PUBLIC_KEY, $request->cookie(FaceIdServiceProvider::FASTLOGIN_COOKIE), ['internal'])),
    ...
    public function login(Request $request, CredentialLoader $credentialLoader, LoginValidator $loginValidator, CredentialRequest $credentialRequest)
    {
        ...
        try {
            $response = $loginValidator->check($request->cookie(FaceIdServiceProvider::FASTLOGIN_COOKIE), $credentials, $requestOptions, $credentialRequest, null, [$requestOptions->getRpId()]);
        } catch (InvalidArgumentException $e) {
            throw new UnauthorizedException('FaceId: Failed validating request', 422, $e);
        }
